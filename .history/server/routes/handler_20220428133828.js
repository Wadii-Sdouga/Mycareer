            ...
            connection.query('SELECT * FROM user WHERE email = ?', [email], function (error, result, fields) {
                ...
                if (result.length == 0) {
                    ...
                    connection.query("INSERT INTO user(idu,nom,email,numTel,mdp ) VALUES (0,?,?,?,?);", [name, email, tel, hashedPassword], function (error, results, fields) {
                        ...
                        const mailOptions = {
                            from: "no-reply@Mobelite.com",
                            to: email,
                            subject: 'Vérification de compte avec Token',
                            text:
                                'Salut,\n\n' +
        ...
        connection.query('SELECT * FROM user WHERE email = ?', [email], function (error, result, fields) {
            ...
            //user exist
                ...
                const mailOptions = {
                    ...
                    to: email,
                    subject: 'Nouveau Token pour mot de passe du compte',
                    text:
                        'Salut,\n\n' +
                        'Veuillez cliquer sur ce lien pour mettre à jour votre mot de passe : \nhttp://' +
                        request.headers.host +
                        '/newpassword/' +
                        token +
                        '.\n'
...
router.get('/dashboard2', (request, response) => {
    ...
    connection.query(' SELECT * FROM user U,carrer C  WHERE U.email= ? ;', [session.email], function (error, result, fields) {
        if (error) {
            console.log(error);
            return response.status(httpCodes.INTERNAL_SERVER_ERROR).send({ msg: error.message });
        }
...
/**************************Upload picture *******************/
router.get('/file-upload', (request, response) => {

    var a = request.request.formData;
    console.log(a);
