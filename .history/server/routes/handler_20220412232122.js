...
router.delete('/admin/user',async (request, response) => {
    console.log("wadii");
    var s = request.body.s;
    console.log(request.body.s);
    connection.query('DELETE FROM user WHERE email=?;', [request.body], function (error, result, fields) {
        if (error) {
            console.log(error);
            return response.status(httpCodes.INTERNAL_SERVER_ERROR).send({ msg: error.message });
        }
