...
console.log(request.body.email);
    connection.query('SELECT * FROM niveau N,user U,technologie T,carrer C WHERE U.email= ? and N.idu=U.idu and N.idt=T.idt and C.idc=U.idc;', [], function (error, result, fields) {
        if (error) {
            console.log(error);
            return response.status(httpCodes.INTERNAL_SERVER_ERROR).send({ msg: error.message });
        }
