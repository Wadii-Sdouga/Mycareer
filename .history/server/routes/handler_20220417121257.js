...
router.get('/admin/add/tech', (request, response) => {
        ...
        connection.query('SELECT titre FROM carrer ;',  function (error, result, fields) {
            if (error) {
                console.log(error);
                return response.status(httpCodes.INTERNAL_SERVER_ERROR).send({ msg: error.message });
            }
...
connection.query("INSERT INTO technologie(idt,nomT,idc) VALUES (0,?,?) where nomT!=f;", [f,result[0].idc], function (error, results, fields) {
    if (error) {
        console.log(error);
        return response.status(httpCodes.INTERNAL_SERVER_ERROR).send({ msg: error.message });
    }
