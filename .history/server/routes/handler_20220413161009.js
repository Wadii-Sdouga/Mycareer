...
router.post('/admin/view/user', (request, response) => {
    var view= request.body.v;
    console.log(request.body);
    router.get('/admin/view/user', (request, response) => {
  
        connection.query("SELECT * FROM user WHERE email=? ;", [view], function (error, results, fields) {
            if (error) {
                console.log(error);
